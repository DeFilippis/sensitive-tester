<template>
    <v-container class="justify-content-center d-flex  flex-column  text-center maincontainer" :style='cont'>
        <v-row>
            <v-col>
        ...
        <v-row>
            <v-col>
                <v-card-title class="d-flex justify-center text-center white--text text-center d-flex justify-content-center " :style="{
                    background: fieldCol,
                    'border-radius': '25px',
                  }">
                    <transition name="fade" mode="out-in" @after-enter="afterEnter" @before-leave="beforeLeave" appear>
        ...
        <v-row>
            <v-col :style="{ visibility: !block ? 'visible' : 'hidden' }">
                <v-btn-toggle v-model="value" class="d-flex justify-content-center" :class="btnFunctionClass" rounded>
                    <v-btn v-for="i in likert" :key="i[0]" @click="answer(i[0])" :style="individualBtnStyle" rounded>
                        {{ i[1] }}
                    </v-btn>
                </v-btn-toggle>
            </v-col>
        ...
        <div data-app>
            <attention-failed :dialog="attentionFailed" @input="attentionFailed = false" :error="attentionError" />
        </div>
    </v-container>
</template>
